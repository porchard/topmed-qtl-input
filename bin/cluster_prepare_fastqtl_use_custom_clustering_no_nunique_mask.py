...
if __name__=='__main__':
    ...
    parser.add_argument('--min_samples', default=1, type=int, help='Minimum number of samples supporting a junction')
    ...
    cmd = f"python3 {os.path.join(args.leafcutter_dir, 'leafcutter_cluster_regtools.py')}" \
        + f' --juncfiles {args.junc_files_list}' \
        + f' --rundir {args.output_dir}' \
        + f' --outprefix {args.prefix}' \
        + f' --minsamples {args.min_samples}' \
    ...
    subprocess.check_call(
        'python3 '+os.path.join('/opt/leafcutter', 'scripts', 'prepare_phenotype_table.py') \
